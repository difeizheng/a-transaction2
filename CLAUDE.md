...
`src/ui/app.py` is the entry. It dynamically imports the selected page module from `src/ui/page_modules/` and calls its `render_sidebar()` (if present) then `render()`. **The page contract is `render()` (+ optional `render_sidebar()`)** — add a page by dropping a module in `src/ui/page_modules/` and adding it to the `PAGES` dict in `app.py`. Large pages are Python **packages** with the same contract (`screener/`, `analysis/` split into focused submodules; `__init__.py` re-exports `render`/`render_sidebar` so imports are unchanged). `src/ui/core.py` holds the shared service singletons (`get_dm` / `get_simulator` / `get_advisor` — one DataManager per process, not one per page), the `page_guard` render error boundary (wired in `app.py`), and A-share color/format helpers (涨红跌绿). `src/ui/components/` holds background workers (`data_worker.py`, `screening_worker.py`) and shared widgets (`step_logger.py`, `service_info.py`, `equity.py` 净值+回撤图).
